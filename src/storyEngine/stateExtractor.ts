...
export const buildStateExtractorPrompt = (chapterNumber: number): string => [
    ...
    `CONTINUITY AND CLUES\nexpectedContinuityConsequences is a hard exact reconciliation list: represent every {id,text} exactly once, never change the id or paraphrase text, and emit no unrelated continuity item. Follow continuityTargets.allowedOperations. For an absent expected id, use open with entry.id/text exactly as expected, entry.establishedChapter=${chapterNumber}, visibility="writer", status="open", and chapter provenance. For an existing target, preserve its canonical text identity and use only the allowed resolve/supersede operation. cluesPlantedIds must be exact open clue entry IDs; cluesPaidOffIds must be exact resolve continuityId values for existing clues. If all continuity target lists are empty, return continuityChanges: [].`,
