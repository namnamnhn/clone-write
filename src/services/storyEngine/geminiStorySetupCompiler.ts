...
export const buildStorySetupCompilerPrompt = (source: string): string => [
    ...
    '- LEGACY V3 CHARACTER TIMING: For every structured characterRegistry entry, preserve unlockChapter as the character baseline availability timing in character.availableFromChapter (or an exact allowedFromChapter/lockedThroughChapter equivalent), create an explicit gates.characters entry whose effective first-allowed chapter exactly equals directAppearanceChapter, and create an explicit gates.pov entry whose effective first-allowed chapter exactly equals povUnlockChapter.',
    '- directAppearanceChapter and povUnlockChapter are separate and MUST NOT be collapsed or treated as interchangeable. Never omit a POV gate merely because the character is already available, never infer an earlier POV, and never invent POV timing when the source provides none.',
    '- If povUnlockChapter is earlier than directAppearanceChapter, preserve both source timings independently. Existing V4 fail-closed semantics will keep effective POV use constrained by both the direct-appearance gate and the POV gate.',
